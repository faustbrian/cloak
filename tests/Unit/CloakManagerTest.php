...
describe('CloakManager', function (): void {
    ...
    test('rethrow recreates original exception class with sanitized message', function (): void {
        $sanitizer = new PatternBasedSanitizer(
            patterns: ['/secret/i'],
            replacement: '[REDACTED]',
        );
        ...
        $rethrown = $manager->rethrow($exception);

        expect($rethrown)->toBeInstanceOf(RuntimeException::class);
        expect($rethrown)->not->toBeInstanceOf(SanitizedException::class);
        expect($rethrown->getMessage())->toBe('This is a [REDACTED] message');
        expect($rethrown->getCode())->toBe(123);
    });

    test('rethrow preserves previous exception chain', function (): void {
        $sanitizer = new PatternBasedSanitizer(
            patterns: ['/secret/i'],
            replacement: '[REDACTED]',
        );
    ...
    test('rethrow returns original when disabled', function (): void {
        config(['cloak.enabled' => false]);
    ...
    test('rethrow returns original when sanitizer does not sanitize', function (): void {
        $sanitizer = new PatternBasedSanitizer(
            patterns: ['/password/i'],
            replacement: '[REDACTED]',
        );
        ...
        $manager = new CloakManager($sanitizer, logOriginal: false);
        $exception = new RuntimeException('This is a safe message');

        $rethrown = $manager->rethrow($exception);

        expect($rethrown)->toBe($exception);
        expect($rethrown->getMessage())->toBe('This is a safe message');
    });

    test('rethrow logs original exception when configured', function (): void {
        Log::spy();
    ...
    test('rethrow logs original exception with request context', function (): void {
        Log::spy();
        ...
        $manager->rethrow($exception, $request);
        ...
        Log::shouldHaveReceived('error')
            ...
            ->with(
                'Original exception before sanitization',
                Mockery::on(fn ($context): bool => isset($context['exception'])
                    && isset($context['url'], $context['method'])
                    && $context['url'] === 'https://example.com/api/test'
                    && $context['method'] === 'GET'),
            );
    });
